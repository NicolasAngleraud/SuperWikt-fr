...
if __name__ == '__main__':
	...
	params_ids = flatten_list([[[f"EXP3LR{k}DP{i}HL{j}" for i in range(len(dropouts))] for j in range(len(hidden_layer_sizes))] for k in range(len(lrs))])
	
	train_examples, freq_dev_examples, rand_dev_examples, freq_test_examples, rand_test_examples = clf.encoded_examples(datafile=args.lexical_data_file)
	...
	for lr in lrs:
		for hidden_layer_size in hidden_layer_sizes:
			for dropout in dropouts:
				...
				clf.training(params, train_examples, freq_dev_examples, rand_dev_examples, classifier, DEVICE, eval_data, clf_file)
				print(f"CLASSIFIER TRAINED ON {len(train_examples)} EXAMPLES...")
				
				classifier = clf.SupersenseTagger(params, DEVICE)
				classifier.load_state_dict(torch.load(clf_file))
				
				clf.evaluation(train_examples, classifier, params, DEVICE, "train", eval_data, 'exp3')
				clf.evaluation(freq_dev_examples, classifier, params, DEVICE, "freq-dev", eval_data, 'exp3')
				clf.evaluation(rand_dev_examples, classifier, params, DEVICE, "rand-dev", eval_data, 'exp3')
				...
				clf.evaluation(freq_test_examples, classifier, params, DEVICE, "freq-test", eval_data, 'exp3')
				clf.evaluation(rand_test_examples, classifier, params, DEVICE, "rand-test", eval_data, 'exp3')

				df_eval.append(eval_data)

	print("CREATION OF THE EVALUATION FILE...")
	df = pd.DataFrame(df_eval)
	excel_filename = f'./exp3/lexical_classifier_results-run{run}.xlsx'
